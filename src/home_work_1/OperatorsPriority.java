...
public class OperatorsPriority {
    public static void main(String[] args) {
        ...
        */
         ...
         * Вначале выполняется тернарная операция в скобках.
         * Каждая из операций во всех частях тернарной операции выполняется согласно таблице приорететов.
         * Значение, ставшее результатом выполнения тернарной операции, затем делится на значение за скобками.
        */
        i = 2;
        j = 8;
         ...
         * 11 / 7 = 1
         * ВНИМАНИЕ!
         * Значения, из которых выбирает тернарный оператор, двух разных типов: boolean и int.
         * Для работы с такой конструкцие можно попытаться использовать переменную с неявной типизацией (var).
         * Однако такая запись приведет к ошибке компиляции. В нашем примере примере после выполнения тернарной операции
